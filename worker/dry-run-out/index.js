    ...
    return c.json(
      {
        ...
        payment: {
          ...
          decimals: 6,
          eip681Uri,
          expiry: Math.floor(Date.now() / 1e3) + 600
        },
        device: {
          id: pn,
          duration,
...
__name(activateRoute, "activateRoute");
...
async function verifyPayment(txHash, env2) {
  const normalizedHash = txHash.toLowerCase();
  const existing = await env2.USED_TX_HASHES.get(normalizedHash);
  if (existing) {
    return { success: false, error: "Transaction already used" };
  }
  try {
    const receipt = await rpc(env2.RPC_URL, "eth_getTransactionReceipt", [txHash]);
    if (!receipt) {
    ...
    const usdcAddress = env2.USDC_CONTRACT.toLowerCase();
    const receiverAddress = env2.RECEIVER_ADDRESS.toLowerCase();
    const requiredAmount = parseUnits(env2.PAYMENT_AMOUNT, 6);
    let validTransfer = false;
    for (const log3 of receipt.logs) {
      if (log3.address.toLowerCase() !== usdcAddress)
        continue;
      if (log3.topics[0] !== ERC20_TRANSFER_TOPIC)
        continue;
      if (log3.topics.length < 3)
        continue;
      const toAddress = "0x" + log3.topics[2].slice(26).toLowerCase();
      if (toAddress !== receiverAddress)
        continue;
      const amount = BigInt(log3.data);
...
async function getLatestBlock(env2) {
  const hex = await rpc(env2.RPC_URL, "eth_blockNumber", []);
  return parseInt(hex, 16);
}
__name(getLatestBlock, "getLatestBlock");
async function pollPayment(sinceBlock, env2) {
  const requiredAmount = parseUnits(env2.PAYMENT_AMOUNT, 6);
  const receiverAddress = env2.RECEIVER_ADDRESS.toLowerCase();
  const receiverPadded = "0x" + receiverAddress.slice(2).padStart(64, "0");
  try {
    const latestBlock = await getLatestBlock(env2);
    if (sinceBlock > latestBlock) {
      return { found: false };
    }
    const logs = await rpc(env2.RPC_URL, "eth_getLogs", [
      {
    ...
    for (const log3 of logs) {
      const txHash = log3.transactionHash.toLowerCase();
      const used = await env2.USED_TX_HASHES.get(txHash);
      if (used)
        continue;
      const amount = BigInt(log3.data);
      if (amount < requiredAmount)
        continue;
      return { found: true, txHash: log3.transactionHash };
    }
    return { found: false };
  } catch (err) {
...
__name(pollPayment, "pollPayment");
...
// src/services/socket-platform.ts
var URI_LOGIN = "/ppr/web/login/login";
var URI_CONTROL_DEVICE = "/dcc/api/auth/web/collector/lite/controlDevice";
var URI_SINGLE_DEVICE_INFO = "/dev/api/auth/valueCloud/dev/singleDeviceInfo";
async function sign(uri, secret) {
  return hmacSha256(uri, secret);
}
__name(sign, "sign");
async function buildAuthHeaders(token, uri) {
  return {
...
async function login(env2) {
  const password = await sha1(env2.SOCKET_API_PASSWORD);
  const url = env2.SOCKET_API_BASE_URL + URI_LOGIN;
  const res = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      account: env2.SOCKET_API_USER,
      password,
      project: "LITE"
    })
  });
  ...
  await env2.AUTH_TOKEN_CACHE.put("shine_token", JSON.stringify(tokenInfo), { expirationTtl: 3600 });
  logger.info("ShineMonitor login success");
  return tokenInfo;
}
__name(login, "login");
async function getToken(env2) {
  const cached = await env2.AUTH_TOKEN_CACHE.get("shine_token", "json");
  if (cached)
    return cached;
  return login(env2);
}
__name(getToken, "getToken");
async function callWithRetry(env2, fn) {
  let token = await getToken(env2);
  try {
    return await fn(token);
  } catch (err) {
    logger.warn("API call failed, re-logging in", { error: err.message });
    await env2.AUTH_TOKEN_CACHE.delete("shine_token");
    token = await login(env2);
    return fn(token);
  }
}
__name(callWithRetry, "callWithRetry");
async function controlDevice(pn, action, env2) {
  const val = action === "ON" ? "1" : "0";
  try {
    return await callWithRetry(env2, async (token) => {
      const url = env2.SOCKET_API_BASE_URL + URI_CONTROL_DEVICE;
...
__name(controlDevice, "controlDevice");
async function getDeviceStatus(pn, env2) {
  const sn = env2.DEFAULT_DEVICE_SN;
  const devaddr = env2.DEFAULT_DEVICE_DEVADDR;
  const devcode = env2.DEFAULT_DEVICE_DEVCODE;
  try {
    return await callWithRetry(env2, async (token) => {
      const query = `?pn=${pn}&sn=${sn}&devaddr=${devaddr}&devcode=${devcode}`;
      const uri = URI_SINGLE_DEVICE_INFO + query;
      const url = env2.SOCKET_API_BASE_URL + uri;
      ...
      if (!device) {
        return { deviceId: pn, online: false, power: "OFF" };
      }
      return {
        deviceId: pn,
...
function verifyRoute(app2) {
  app2.post("/verify", async (c) => {
    ...
    await c.env.USED_TX_HASHES.put(txHash.toLowerCase(), Date.now().toString(), { expirationTtl: 2592e3 });
    ...
    return c.json({
      ...
      device: {
        ...
        status: "ON",
        duration,
        shutdownAt: new Date(Date.now() + duration * 1e3).toISOString()
      },
      shutdownToken,
      txHash
    });
...
__name(verifyRoute, "verifyRoute");

// src/routes/status.ts
function statusRoute(app2) {
...
__name(statusRoute, "statusRoute");

// src/routes/poll-payment.ts
function pollPaymentRoute(app2) {
  app2.get("/latest-block", async (c) => {
    try {
...
__name(pollPaymentRoute, "pollPaymentRoute");

// src/routes/turn-off.ts
function turnOffRoute(app2) {
  app2.post("/turn-off", async (c) => {
    const { token, pn } = await c.req.json();
    if (!token || !pn) {
      return c.json({ error: "token and pn are required" }, 400);
    }
    const kvKey = `shutdown:${pn}:${token}`;
    const record = await c.env.SHUTDOWN_TOKENS.get(kvKey, "json");
    if (!record) {
...
__name(turnOffRoute, "turnOffRoute");
